...
Almost every module under `lua/config/` and `lua/plugins/` returns a **function that takes `config` and mutates it in place** (side effects on the shared config table), rather than returning a value.

`safe_require` also accepts a **table with an `apply` field** of that same shape, and calls it the same way. That form is for modules which have to export something as well — actions for `keyboard.lua`, entries for `palette.lua` — because a Lua function cannot carry fields, so a module that must be both a function and a table otherwise has to become two files. `kabegami.lua` is the one that needs it. Prefer the bare function unless there is something to export.

The remaining exceptions are `lua/config/global.lua`, `lua/config/share_mode.lua`, `lua/config/palette.lua` and `lua/utils/log.lua`, which return plain tables with no `apply` and are required directly rather than through `safe_require`.
...
Split by concern — `colors`, `appearance`, `window`, `background`, `fonts`, `keyboard`, `mouse`, `programs`, `menu`, `kabegami`, `share_mode`, `palette`. Load order in `wezterm.lua` is intentional; keep new modules in a sensible position. `kabegami.lua` (wallpaper) is one file that both applies and exports: `M.apply` only sets an image on human-rights hosts, only when there is one to apply, and sets `window_background_image` and nothing else, while the actions beside it are what `keyboard.lua` and `palette.lua` bind. It is one file *because* `safe_require` accepts the table-with-`apply` form — see Module convention. Splitting it to satisfy a bare-function contract produced two files that read as duplicates of each other.

`background.lua` owns how the background renders: `window_background_opacity` and `window_background_image_hsb`. The two live together because they are the same kind of knob — both decide how readable text stays over whatever is behind it — and keeping them apart had spread `window_background_*` across three files. It applies unconditionally: opacity matters on every host, and an `hsb` without an image is inert, so it never has to know whether there is a wallpaper. That leaves `window.lua` holding the window frame alone.

Two wallpaper modes: **fixed** keeps one image, **random** draws from `~/.kabegami/random/` and picks something different from what is showing on every press. The split is not invented here — i3 already reads the same directory with `setwallpaper <file>` and `setrandom ~/.kabegami/random`.

The single source of truth is **`~/.kabegami/mode`**, one line: either `random`, or a path relative to `~/.kabegami/` such as `random/azusa_by_namatume.png` or `azusa_02.jpg`. A file, not `wezterm.GLOBAL`, because GLOBAL does not survive a restart. Paths are relative and always written with forward slashes so the same file means the same thing read from Windows (where WezTerm runs) and from WSL (where an editor plugin would write it). Missing, empty or unreadable falls back to `M.default_state`. Three ways to write it, all equivalent: `LEADER-r` (random), `LEADER-f` (pin what is on screen), `LEADER-p` (pick from a list of every image under `~/.kabegami/`), and editing the line by hand is supported on purpose. `wezterm.GLOBAL.kabegami_image` still exists, but only holds *which* image random mode drew for this session, so that an unrelated reload does not reshuffle the wallpaper.
...
`share_mode.lua` returns a plain table of actions with no `apply` (like `utils/log.lua`), so `keyboard.lua` requires it directly. `share_mode.lua` backs the screen-share toggles (`LEADER-s` all, `LEADER-o` opacity, `LEADER-b` wallpaper) by flipping flags in `wezterm.GLOBAL` and calling `wezterm.reload_configuration()`. Consequently `background.lua` and `kabegami.lua` read those flags via `global.is_opaque()` / `global.is_kabegami_disabled()` instead of hardcoding — do not "simplify" the opacity back to a constant. The reload-based design is deliberate: `window:set_config_overrides()` cannot unset `window_background_image` once the config file sets it (wezterm/wezterm#5240).

`palette.lua` owns the **single** `augment-command-palette` handler, which is why it is the one module `wezterm.lua` requires for its side effect. Feature modules expose a `palette_entries` table and know nothing about the palette; `palette.lua` concatenates them and knows nothing about what they do. Registering from one place means the question of what WezTerm does with the return values of two handlers never has to be answered. Add entries by adding to a feature module's `palette_entries` and listing that module in `palette.lua`'s `sources`.

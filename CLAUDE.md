...
Split by concern — `colors`, `appearance`, `window`, `background`, `fonts`, `keyboard`, `mouse`, `programs`, `menu`, `kabegami`, `kabegami_mode`, `share_mode`. Load order in `wezterm.lua` is intentional; keep new modules in a sensible position. `kabegami.lua` (wallpaper) only applies an image on human-rights hosts, and only when there is one to apply — it sets `window_background_image` and nothing else.
...
`kabegami_mode.lua` decides which image that is, and is the second module returning a plain table rather than a `function(config)`. Two modes: **fixed** keeps one image, **random** draws from `~/.kabegami/random/` and picks something different from what is showing on every press. The split is not invented here — i3 already reads the same directory with `setwallpaper <file>` and `setrandom ~/.kabegami/random`.

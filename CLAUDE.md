...
Split by concern — `colors`, `appearance`, `window`, `background`, `fonts`, `keyboard`, `mouse`, `programs`, `menu`, `kabegami/`, `share_mode`, `palette`. Load order in `wezterm.lua` is intentional; keep new modules in a sensible position. The wallpaper is a **directory module**, `lua/config/kabegami/`, the same shape `lua/plugins/tabline-wez/` uses: `init.lua` is what `require("config.kabegami")` resolves to and keeps the `function(config)` contract, and everything that is not "put the image into config" sits beside it. `init.lua` only applies an image on human-rights hosts, only when there is one to apply, and sets `window_background_image` and nothing else.
...
`kabegami/mode.lua` decides which image that is, and returns a plain table rather than a `function(config)`. Two modes: **fixed** keeps one image, **random** draws from `~/.kabegami/random/` and picks something different from what is showing on every press. The split is not invented here — i3 already reads the same directory with `setwallpaper <file>` and `setrandom ~/.kabegami/random`.
...
`share_mode.lua` and `kabegami/mode.lua` are the exceptions to the `function(config)` convention: they return plain tables of actions (like `utils/log.lua`), so `keyboard.lua` requires them directly rather than through `safe_require`. `share_mode.lua` backs the screen-share toggles (`LEADER-s` all, `LEADER-o` opacity, `LEADER-b` wallpaper) by flipping flags in `wezterm.GLOBAL` and calling `wezterm.reload_configuration()`. Consequently `background.lua` and `kabegami/init.lua` read those flags via `global.is_opaque()` / `global.is_kabegami_disabled()` instead of hardcoding — do not "simplify" the opacity back to a constant. The reload-based design is deliberate: `window:set_config_overrides()` cannot unset `window_background_image` once the config file sets it (wezterm/wezterm#5240).
